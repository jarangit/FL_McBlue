...
const LoginModal = ({ handleClose, open, onSubmit, onChangeDataForm }) => {
  const styled = {
          ...
          >
            <div className='flex flex-col items-center gap-4 relative'>
              ...
              <div className='w-full'>
                <form action="" className='flex flex-col gap-4 w-full' onChange={(e) => onChangeDataForm(e)}>
                  <input type="text" placeholder='UserName' className={`${styled.input}`} name ="username"/>
                  <input type="password" placeholder='รหัสผ่าน' className={`${styled.input}`} name ="password"/>
                </form>
              </div>
              <button className={`${styled.but_yellow}`} onClick={onSubmit}>เข้าสู่ระบบ</button>
